...
class ToolManager:
    ...
    def load_tools(self, tools_dir: str = "", config_dict=None):
        ...
        if tools_dir:
            ...
            if tools_path.exists():
                self._load_tools_from_directory(str(tools_path))
            else:
                logger.warning(
                    "Tool directory '%s' not found. Falling back to package tool discovery.",
                    tools_dir,
                )
    ...
    def create_tool(self, name: str, config_override: Dict[str, Any] = None) -> BaseTool:
        """
        Get a new instance of a tool by name.

        :param name: The name of the tool to get.
        :param config_override: Optional runtime config override for this tool instance.
        ...
        if tool_class:
            ...
            # Apply base config from config.yaml first.
            base_config = {}
            if hasattr(self, "tool_configs") and name in self.tool_configs and isinstance(self.tool_configs[name], dict):
                base_config = self.tool_configs[name]

            # Then merge runtime override for this execution (if any).
            override = config_override if isinstance(config_override, dict) else {}
            if base_config or override:
                merged = dict(base_config)
                merged.update(override)
                tool_instance.config = merged
