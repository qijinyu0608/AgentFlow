...
class BrowserTool(BaseTool):
    ...
    async def _init_browser(self) -> BrowserContext:
        ...
        runtime_config = self.config if isinstance(getattr(self, "config", None), dict) else {}
        headless = bool(runtime_config.get("headless", _header_less()))
        highlight_elements = bool(runtime_config.get("highlight_elements", True))
        config_signature = json.dumps(
            {"headless": headless, "highlight_elements": highlight_elements},
            ensure_ascii=False,
            sort_keys=True,
        )

        if BrowserTool._initialized and BrowserTool._config_signature != config_signature:
            try:
        ...
        if not BrowserTool._initialized:
            ...
            BrowserTool.browser = Browser(BrowserConfig(headless=headless,
                                                        disable_security=True))
            context_config = BrowserContextConfig()
            context_config.highlight_elements = highlight_elements
            BrowserTool.browser_context = await BrowserTool.browser.new_context(context_config)
            BrowserTool._initialized = True
            BrowserTool._config_signature = config_signature
